...
class DataFramePlotterApp:
    def __init__(self, root):
        ...
        self.model_frame.grid(row=4, column=1, padx=10, pady=10, sticky="nsew")
        ...
        def get_selected(choice):
            """
            Purpose:
                Update the selected model type and toggle the visibility of the "Execute Model" button.
            ...
            if self.fitModelChoice != "Fit Model":
                self.execute_button.grid(row=1, padx=5, pady=5)
            else:
                self.execute_button.grid_remove()

        self.modelOptions = [
            'Viterbi --> HMM',
            'Viterbi --> Time-Dependent Pseudo-HMM',
    ...
    def create_hsmm_parameters_section(self):
        ...
        self.hsmm_frame.grid(row=6, column=0, columnspan=3, padx=5, pady=5, sticky="nsew")

        header_label = ttk.Label(self.hsmm_frame, text="Duration Distribution Parameters", font=("Arial", 12, "bold"))
        header_label.grid(row=0, column=0, columnspan=4, pady=10, sticky="n")

        if self.number_of_states > 5:
            canvas = tk.Canvas(self.hsmm_frame, height=200)
            canvas.grid(row=2, column=0, columnspan=4)
            scrollbar = ttk.Scrollbar(self.hsmm_frame, orient="vertical", command=canvas.yview)
            scrollbar.grid(row=2, column=4, sticky="ns")
            self.hsmm_inner_frame = ttk.Frame(canvas)
            canvas.create_window((0, 0), window=self.hsmm_inner_frame, anchor="nw")
            canvas.configure(yscrollcommand=scrollbar.set)
        else:
            self.hsmm_inner_frame = self.hsmm_frame

        headers = ["State", "d", "v", "s"]
        for col, header in enumerate(headers):
            header_label = ttk.Label(self.hsmm_inner_frame, text=header, font=("Arial", 10, "bold"))
            header_label.grid(row=1, column=col, padx=5, pady=5)

        self.hsmm_entries_d = []
        self.hsmm_entries_v = []
        self.hsmm_entries_s = []
        self.hsmm_parameters_d = [None] * self.number_of_states
        self.hsmm_parameters_v = [None] * self.number_of_states
        self.hsmm_parameters_s = [None] * self.number_of_states

        for i in range(self.number_of_states):
            state_label = ttk.Label(self.hsmm_inner_frame, text=f"State-{i}")
            state_label.grid(row=i + 2, column=0, padx=5, pady=5)

            entry_d = ttk.Entry(self.hsmm_inner_frame)
            entry_d.grid(row=i + 2, column=1, padx=5, pady=5)
            self.hsmm_entries_d.append(entry_d)

            entry_v = ttk.Entry(self.hsmm_inner_frame)
            entry_v.grid(row=i + 2, column=2, padx=5, pady=5)
            self.hsmm_entries_v.append(entry_v)

            entry_s = ttk.Entry(self.hsmm_inner_frame)
            entry_s.grid(row=i + 2, column=3, padx=5, pady=5)
            self.hsmm_entries_s.append(entry_s)

            confirm_button = ttk.Button(self.hsmm_inner_frame, text="Confirm", command=lambda idx=i: self.confirm_hsmm_row(idx))
            confirm_button.grid(row=i + 2, column=4, padx=5, pady=5)

    def confirm_hsmm_row(self, idx):
        try:
            d = float(self.hsmm_entries_d[idx].get())
            v = float(self.hsmm_entries_v[idx].get())
            s = float(self.hsmm_entries_s[idx].get())
            if any(param <= 0 for param in [d, v, s]):
                raise ValueError("All parameters must be positive.")

            self.hsmm_parameters_d[idx] = d
            self.hsmm_parameters_v[idx] = v
            self.hsmm_parameters_s[idx] = s

            messagebox.showinfo("Success", f"HSMM Parameters for State-{idx} confirmed!")
        except ValueError as e:
            messagebox.showerror("Error", str(e))
    ...
    def create_transition_matrix_section(self):
        ...
        for i in range(self.number_of_states):
            label = ttk.Label(
                parent_frame, text=f"State-{i} Transition Probabilities (comma separated):"
            )
            ...
            button.grid(row=2 * i + 1, column=1, padx=5, pady=5, sticky="ew")
    ...
    def toggle_uniform_probabilities(self):
        ...
        if self.uniform_var.get():
            ...
            self.init_prob_entry.insert(0, "0.0")
    ...
    def execute_model(self):
        """
        Purpose:
            Execute the selected model on the DataFrame's selected columns and display results.
        ...
        if not self.fitModelChoice:
            messagebox.showerror("Error", "Please select a model to execute!")
            return
        ...
        try:
            ...
            ax.plot(x_data, fitted_y, label="Fitted Model", color="red", linewidth=2)
    ...
    def run_model(self, y_data):
        ...
        if self.fitModelChoice == 'Viterbi --> HMM':
        ...
        elif self.fitModelChoice == 'Viterbi --> Time-Dependent Pseudo-HMM':
            self.sample_rate_label = ttk.Label(self.processing_frame, text="Sample Rate (Hz):")
            self.sample_rate_label.grid(row=8, column=0, padx=5, pady=5)

            self.sample_rate_entry = ttk.Entry(self.processing_frame)
            self.sample_rate_entry.grid(row=8, column=1, padx=5, pady=5)

            self.sample_rate_confirm_button = ttk.Button(
                self.processing_frame, text="Confirm", command=self.confirm_sample_rate
            )
            self.sample_rate_confirm_button.grid(row=8, column=2, padx=5, pady=5)




            fitted_states, fitted_y = self.run_timedep_pseudohmm(y_data, self.sample_rate)
            return {'State Estimates' : fitted_states, 'Emission Estimates' : fitted_y}
        
        elif self.fitModelChoice == 'Viterbi --> Averaged Chunked HMM':
            self.run_avg_chunked_hmm(y_data)
            ...
            self.run_fwdbwd_hmm(y_data)
        elif self.fitModelChoice == 'Viterbi ---> 2-Stage Changepoint-Vector-State HMM':
           self. run_2stage_changepoint_vector_state_hmm(y_data)
            
            

        return fitted_states, fitted_y
    ...
    def get_hsmm_params(self):
        """Retrieve HMM parameters from the Model Frame for use in run_viterbi_hmm."""
        return self.initial_probabilities, self.state_means, self.state_stds, self.transition_matrix, self.hsmm_parameters_d, self.hsmm_parameters_s
             



    def run_viterbi_hmm(self, y_data):
        start_prob, means, stds, transition_matrix = self.get_hsmm_params()
    ...
    def run_timedep_pseudohmm(self, y_data, sample_rate):
    ...
    def run_hsmm(self, y_data, sample_rate):
        ...
        viterbi_out = self.viterbi_hsmm(self, y_data, states, start_prob, transition_matrix, min_duration, max_duration, sample_rate, cdf_table)
    ...
    def viterbi_algorithm_vector(obs, states, start_prob, transition_matrix, gamma, tol=1e-8):
    ...
    def viterbi_hsmm(self, obs, states, start_prob, transition_matrix, min_duration, max_duration, sample_rate, cdf_lookup):
        ...
        print(log_viterbi_table[-1])
        print(valid_state_table[-1][best_state])
